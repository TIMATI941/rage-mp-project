...
namespace NeptuneEvo.Core
{
    class Commands : Script
    {
        ...
        //try tz
        public static void SendToGangs(string message) {
            foreach (var p in Main.Players.Keys.ToList()) {
                if(!Main.Players.ContainsKey(p)) continue;
                if(Main.Players[p].FractionID != 1 || Main.Players[p].FractionID != 2 || Main.Players[p].FractionID != 3 || 
                Main.Players[p].FractionID != 4 || Main.Players[p].FractionID != 5) continue;
                p.SendChatMessage(message);
            }
        }
		...
		[Command("savefveh")]
        public static void CMD_savefveh(Client client, string number, string model, int fraction, int rank)
        {
            try
            {
                if (!Group.CanUseCmd(client, "savefveh")) return;

                if (Fractions.Configs.FractionVehicles[fraction].ContainsKey(number))
                {
                    Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"Там уже есть фракционные машины с этим идентификатором", 3000);
                    return;
                }

                string pos = JsonConvert.SerializeObject(client.Vehicle.Position + new Vector3(0, 0, 1));
                string rot = JsonConvert.SerializeObject(client.Vehicle.Rotation);

                int color1 = client.Vehicle.PrimaryColor;
                int color2 = client.Vehicle.SecondaryColor;
                ...
                client.SendChatMessage($"{number} was saved");
            }
            catch { }
        }

        [Command("delfveh")]
        public static void CMD_deletefveh(Client client, string number)
        {
            try
            {
                if (!Group.CanUseCmd(client, "savefveh")) return;
        ...
        [Command("savejveh")]
        public static void CMD_savejveh(Client client, string number, string model, int type)
        {
            try
            {
                if (!Group.CanUseCmd(client, "savejveh")) return;
        ...
        [Command("deljveh")]
        public static void CMD_deletejveh(Client client, string number)
        {
            try
            {
                if (!Group.CanUseCmd(client, "savejveh")) return;
        ...
        //try tz
        [Command("acceptrob")]
        public static void CMD_acceptrob(Client player) {
            try {
                foreach(Business biz in BusinessManager.BizList.Values) {
                    if(biz.Rubbish != true) continue;

                    Trigger.ClientEvent(player, "createWaypoint", biz.EnterPoint.X, biz.EnterPoint.X);
                    break;
                }
